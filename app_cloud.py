...
def transcribe_with_groq_api(api_key, file_path, language, initial_prompt=""):
    ...
    with open(file_path, "rb") as f:
        ...
        data = {
            ...
            "language": language,
            "timestamp_granularities[]": "segment" # Groq API uses segments reliably
        }
        if initial_prompt:
    ...
    # המרת התוצאה לפורמט שהאפליקציה שלך צריכה
    words_list = []
    word_id = 0
    
    # חלוקה גסה של זמן המקטעים למילים (כי ה-API לא תמיד מחזיר תזמון מילה-במילה)
    for seg in api_json.get("segments", []):
        ...
        for i, raw_word in enumerate(seg_text):
            ...
            words_list.append({
                ...
                "confidence": 0.99, # הערכת ברירת מחדל ל-API
                "speaker": "0", 
                "deleted": False
            })
            word_id += 1
...
def auto_punctuate_with_gemini(api_key, words_data):
    ...
    try:
        ...
        # בונים רשימה ממוספרת לחיסכון באסימונים ומניעת הזיות
        ...
        # עכשיו התשובה מובטחת להיות JSON נקי, אין צורך ב-Regex
        return json.loads(response.text)
        
    except Exception as e:
        # אפשר גם להוסיף הדפסה ל-Log במקרה של שגיאה כדי להבין מה קרה
...
# ==========================================
st.set_page_config(page_title="Advanced STT - Local Whisper", layout="wide")
...
st.title("🎙️ מערכת תמלול מקומית - Faster Whisper")
st.markdown("תמלול מהיר על גבי המחשב המקומי, ותיקון שגיאות אינטראקטיבי.")
...
with st.sidebar:
    ...
    enable_diarization = st.toggle("הפעל חלוקה לדוברים מראש", value=False)
    expected_speakers = st.number_input("מספר משתתפים צפוי (0 = זיהוי אוטומטי):", min_value=0, max_value=20, value=0, step=1)
    ...
    diarize_option = st.selectbox(
        ...
        help="לחץ על כל אחת מהאופציות כדי לראות את היתרונות, החסרונות, והעלויות שלה. מתחת לכל אופציה יופיע גם קישור ישיר ליצירת מפתח ה-API המתאים."
    )
    ...
    # שדות דינמיים וקישורים לפי הבחירה
    api_key_diarize = ""
    
    
        
    if "Deepgram" in diarize_option:
        st.info("💡 **Deepgram API:**\n* **יתרון:** מהיר מאוד ומדויק. פתרון אידיאלי לענן. נותן 200$ קרדיט חינמי בהרשמה ראשונית (מספיק לאלפי שעות).\n* **חיסרון:** דורש הרשמה ופתיחת משתמש אצלם.")
        api_key_diarize = st.text_input("מפתח API של Deepgram:", type="password", help="הזן כאן את המפתח שיצרת באתר Deepgram.")
        st.caption("[להרשמה והוצאת מפתח חינם מ-Deepgram לחץ כאן](https://console.deepgram.com/)")
        
    elif "Replicate" in diarize_option:
        st.info("💡 **Pyannote 3.1 בענן (Replicate):**\n* **יתרון:** דיוק מילי-שניות מקסימלי, זהה לחלוטין למודל המקומי.\n* **חיסרון:** גובה כמה סנטים בודדים לכל דקת שמע לאחר סיום הקרדיט החינמי הראשוני.")
        api_key_diarize = st.text_input("מפתח API של Replicate:", type="password", help="הזן כאן את המפתח שיצרת באתר Replicate.")
        st.caption("[להרשמה והוצאת מפתח מ-Replicate לחץ כאן](https://replicate.com/account/api-tokens)")
        
    elif "Gemini" in diarize_option:
        st.info("💡 **Gemini 1.5:**\n* **יתרון:** חינמי לחלוטין דרך Google AI Studio.\n* **חיסרון:** מבוסס על הבנת שפה ולכן דיוק התזמונים (במילי-שניות) נמוך יותר משמעותית לעומת מנועי אודיו ייעודיים.")
        api_key_diarize = st.text_input("מפתח API של Gemini:", type="password", key="gemini_diarize", help="הזן כאן את המפתח של Google AI Studio.")
        st.caption("[להוצאת מפתח חינם מ-Google AI Studio לחץ כאן](https://aistudio.google.com/app/apikey)")
        ...
        if st.button("🚀 הפעל זיהוי דוברים עכשיו", use_container_width=True, type="primary"):
            if not api_key_diarize: 
                ...
                with st.spinner(f"מנתח קולות באמצעות {diarize_option.split(' ')[0]}..."):
                    try:
                        # ===== לוגיקה למקומי (Pyannote המקורי שלך) =====
                        
                        # ===== לוגיקה לענן (Deepgram) =====
                        if "Deepgram" in diarize_option:
                            turns = diarize_with_deepgram(api_key_diarize, st.session_state.local_audio_path, expected_speakers)

                        # ===== לוגיקה לענן (Replicate / Pyannote) =====
                        elif "Replicate" in diarize_option:
                            turns = diarize_with_replicate(api_key_diarize, st.session_state.local_audio_path, expected_speakers)
                            
                        # ===== לוגיקה לענן (Gemini) =====
                        elif "Gemini" in diarize_option:
                            st.warning("זיהוי דוברים דרך Gemini עדיין בפיתוח לגרסה זו. אנא בחר מנוע אודיו ייעודי.")
                            turns = [] # כאן נוכל לשלב פונקציית פרומפט ייעודית בהמשך

                        # עדכון המילים בעורך בהתאם לתוצאות החלוקה (משותף לכולם)
                        if turns:
    ...
    st.divider()
    if st.button("איפוס מערכת (Clear Data)", help="מנקה את הזיכרון הזמני של האפליקציה ומחזיר אותה למצב התחלתי (לא מוחק פרויקטים שמורים)."):
        st.session_state.clear()
        st.rerun()

# ==========================================
# שלב 1: הזנת אודיו ותמלול
# ==========================================
...
with col_upload:
    uploaded_file = st.file_uploader(
        ...
        help="תומך ברוב קובצי השמע. קבצים כבדים מאוד עשויים לקחת זמן עיבוד ארוך יותר. עובד הכי טוב עם WAV"
    )
    
    st.markdown("<br>", unsafe_allow_html=True)
    with st.container(border=True):
        use_cloud_api = st.toggle(
            "☁️ תמלול מהיר בענן (למחשבים ללא כרטיס מסך חזק)", 
            value=False,
            help="אם המחשב שלך איטי או נתקע בזמן תמלול, הפעל אפשרות זו. היא תשלח את הקובץ לשרת חיצוני חזק ותחזיר תמלול תוך שניות בודדות."
        )
        api_key = ""
        if use_cloud_api:
            api_key = st.text_input("מפתח API של Groq:", type="password", placeholder="gsk_...", help="נדרש מפתח חינמי מ-Groq.com כדי להשתמש בשרתי הענן שלהם.")
            st.caption("[להוצאת מפתח חינם בשניות לחץ כאן](https://console.groq.com/keys)")

    st.markdown("<br>", unsafe_allow_html=True)
    transcribe_clicked = st.button("🚀 תמלל עכשיו", type="primary", use_container_width=True, help="מתחיל את תהליך זיהוי הדיבור והמרתו לטקסט בהתאם להגדרות שבחרת.")
HAS_GPU = False
SYSTEM_DEVICE = "cloud (api)"
with col_lang:
    st.write("הגדרות תמלול:")
    language_choice = st.radio("שפה", ["he", "en"], format_func=lambda x: "עברית" if x == "he" else "אנגלית", horizontal=True, help="הגדרת השפה המדוברת באודיו משפרת דרסטית את דיוק המודל ומונעת הזיות טקסט.")
    enable_translation = st.toggle("🌐 תרגם הכל לאנגלית", value=False, help="אם האודיו בעברית, המודל יתמלל ויתרגם אותו ישירות לאנגלית באותו הזמן.")
    
    default_model = "ivrit-ai/whisper-large-v3-ct2" if HAS_GPU else "small"
    model_options = {
        "small": "Small (ל-CPU)",
        "medium": "Medium (פשרה ל-CPU)",
        "large-v3": "Large-v3 (לאנגלית - GPU)",
        "ivrit-ai/whisper-large-v3-ct2": "Ivrit.AI (לעברית - GPU)"
    }
    selected_model_key = st.selectbox(
        ...
        help="מודלים גדולים יותר (לארג'/עברית) מדויקים יותר אך דורשים כרטיס מסך חזק. מודלים קטנים רצים היטב על מעבד רגיל אך עלולים לפספס מילים."
    )
    ...
    initial_prompt = st.text_area(
        ...
        height=80,
        help="המודל לעיתים מתקשה עם שמות פרטיים, סלנג או מונחים מקצועיים. הקלד אותם כאן כדי 'ללמד' אותו איך לכתוב אותם נכון (למשל: תל אביב-יפו, פייתון, סטרימליט)."
    )
...
if transcribe_clicked and st.session_state.audio_bytes:
    if not api_key:
        st.error("יש להזין מפתח API של Groq כדי לתמלל בענן.")
    else:
        try:
            with st.spinner("מתמלל בשרתי הענן של Groq... (זה לוקח שניות בודדות)"):
                words_list, duration = transcribe_with_groq_api(api_key, st.session_state.local_audio_path, language_choice, initial_prompt)
            ...
            # שמירה והגדרת הפרויקט (משותף לשתי השיטות)
            st.session_state.words_data = words_list
            st.session_state.original_words_data = copy.deepcopy(words_list)
...
החזר לי את התמלול המתוקן בלבד."""
            ...
            # הצגת הטקסט בתוך בלוק עם כפתור העתקה
            st.code(correction_prompt, language="text")

        # ----------------------------------------------------
        # כאן ממשיך הקוד הקיים של הסטטיסטיקות:
        # with tab_stats:
        #    stats = {}
        # ...
        # טאב סטטיסטיקות
    ...
    with col_speakers:
        with st.expander("👥 מיזוג ושינוי שמות דוברים (Bulk)", expanded=False):
            st.caption("כדי למזג, פשוט תן לשני דוברים את אותו השם (לדוגמה: 'אילון').")
    ...
    # הוספת הנגן המקורי של Streamlit שתומך בחיפוש קדימה ואחורה באופן מובנה
    ...
    # עריכת מילה ספציפית (לחיצה כפולה) - הועבר לכאן למטה כדי למנוע קפיצות במסך!
